...
export const addFunds = async (req, res) => {
  try {
    const { userId, amount, paymentMethodId, gatewayTxnId } = req.body;
    if (!userId || !amount || amount <= 0) return res.status(400).json({ message: 'Invalid payload' });

    const user = await EnhancedUser.findById(userId);
    if (!user) return res.status(404).json({ message: 'User not found' });

    // If paymentMethodId provided, validate it belongs to user
    let pm = null;
    if (paymentMethodId) {
...
//         registration.paymentId = payment.id; // store payment_id
...
//         console.log("✅ Payment captured, registration updated:", registration._id);
//       }
//     }
...
//         await registration.save();
//         console.log("❌ Payment failed, registration updated:", registration._id);
//       }
//     }
