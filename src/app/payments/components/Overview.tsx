...
export default function Overview({ data = sampleData }: OverviewProps) {
      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            ...
            // Refetch balance data to update the UI
            console.log("Refetching balance after successful payment...");
            await refetchBalance();
            console.log("Balance refetch completed");
            ...
            if (error?.data?.code === "DAILY_TRANSACTION_LIMIT_EXCEEDED") {
              alert(
                `Daily transaction limit exceeded!\n\nLimit: ₹${error.data.limit.toLocaleString()}\nUsed: ₹${error.data.used.toLocaleString()}\nRemaining: ₹${error.data.remaining.toLocaleString()}`
              );
            } else {
              alert(
                error?.data?.message || "Failed to add funds. Please try again."
              );
            }
          }
        },
    ...
    try {
      ...
      alert(
        `Withdrawal request submitted successfully!\n\n` +
          ...
          `Available Balance: ₹${result.wallet.availableBalance.toFixed(
            2
          )}\n\n` +
          `${result.note}`
      );
      ...
      if (error?.data?.code === "INSUFFICIENT_BALANCE") {
        alert(
          `Insufficient balance!\n\nAvailable: ₹${error.data.availableBalance}\nRequested: ₹${error.data.requestedAmount}`
        );
      } else if (error?.data?.code === "BANK_ACCOUNT_NOT_VERIFIED") {
        alert(
          "Bank account must be verified before withdrawals. Please verify your bank account first."
        );
      } else if (error?.data?.code === "DAILY_WITHDRAWAL_LIMIT_EXCEEDED") {
        alert(
  ...
  const clearedEventRevenue = 2500.0;
  const podiumPlatformFee = 250.0;
            ...
            <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
              ...
              <div>
                ...
                <div className="flex justify-between text-sm text-gray-600 mt-2">
                  ...
                  <span>
                    Maximum: ₹{(balance?.availableBalance || 0).toFixed(2)}
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  >
                    25% ₹{((balance?.availableBalance || 0) * 0.25).toFixed(2)}
                  </button>
                  <button
                    onClick={() => handleQuickSelect(50)}
                    className="px-3 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
                  >
                    50% ₹{((balance?.availableBalance || 0) * 0.5).toFixed(2)}
                  </button>
                  <button
                    onClick={() => handleQuickSelect(75)}
                    className="px-3 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors"
                  >
                    75% ₹{((balance?.availableBalance || 0) * 0.75).toFixed(2)}
                  </button>
                  <button
                    onClick={handleMaxWithdraw}
                    className="px-3 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors bg-gray-100"
                  >
                    Max ₹{(balance?.availableBalance || 0).toFixed(2)}
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                {paymentMethods &&
                paymentMethods.filter((pm: any) => pm.type === "bank").length >
                  0 ? (
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className="text-sm font-medium text-gray-900">
                              {bankAccount.details?.bankName || "Bank"} ****{" "}
                              {bankAccount.details?.accountNumber?.slice(-4) ||
                                "****"}
                            </div>
                            <div className="text-xs text-gray-600">
                              {bankAccount.details?.accountType || "Savings"}{" "}
                              Account
              ...
              {withdrawAmount &&
                (parseFloat(withdrawAmount) < 10 ||
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span>Please enter a valid withdrawal amount.</span>
                  </div>
                )}
            </div>
            ...
            <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2 sm:gap-3 p-4 sm:p-6 border-t border-gray-100">
              ...
              <button
                ...
                disabled={
                  ...
                  !selectedBankAccount ||
                  parseFloat(withdrawAmount) < 10 ||
                  parseFloat(withdrawAmount) > (balance?.availableBalance || 0)
                }
...
const sampleData: PaymentData = {
  ...
  dailyWithdrawalLimit: 1, // ₹1 lakh
  dailyTransactionLimit: 10, // ₹10 lakh
};
