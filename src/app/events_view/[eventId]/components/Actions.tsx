...
import { usePostponeEventMutation, useGetPostponementOptionsQuery } from '@/store/slices/enhancedEventSlice';
...
const Actions: React.FC<ActionsProps> = ({ eventId }) => {
  ...
  const { data: postponementOptions } = useGetPostponementOptionsQuery(eventId, {
    skip: !showPostponeModal // Only fetch when modal is open
  });
  ...
  const handleNewMeetingDataChange = (field: keyof typeof newMeetingData, value: string) => {
  ...
  const handlePostponeEvent = async () => {
    ...
    if (checkboxes.newDates && (!newEventData.startDate || !newEventData.endDate)) {
    ...
    try {
      ...
      // Add location if provided (for offline/hybrid events)
      if (newEventData.location) {
        postponementData.newLocation = newEventData.location;
      }

      // Add meeting details if provided (for online/hybrid events)
      ...
      if (result.success) {
        ...
        setError(result.message || 'Failed to postpone event');
      }
    } catch (error: unknown) {
      ...
      if (error && typeof error === 'object' && 'data' in error) {
        const errorData = (error as { data?: { errors?: string[]; message?: string } }).data;
        ...
        if (errorData?.errors && Array.isArray(errorData.errors) && errorData.errors.length > 0) {
          ...
          } else if (specificError.includes('Minimum') && specificError.includes('days notice')) {
            ...
            <div className="p-6">
              ...
              {postponementOptions?.data && (
                <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <h4 className="text-sm font-medium text-blue-800 mb-2">Policy Constraints</h4>
                  <div className="grid grid-cols-2 gap-4 text-xs text-blue-700">
                    <div>• Minimum notice: {postponementOptions.data.noticeRequired} days</div>
                    <div>• Max duration: {postponementOptions.data.maxPostponementDuration} days</div>
                    <div>• Refund allowed: {postponementOptions.data.offerRefundOnPostponement ? 'Yes' : 'No'}</div>
                    <div>• Max refund: {postponementOptions.data.maxRefundPercentage}%</div>
                  </div>
                </div>
              )}
                ...
                <div className="mb-6 bg-red-50 border border-red-200 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h4 className="text-sm font-medium text-red-800 mb-1">Unable to Postpone Event</h4>
                    ...
                    {checkboxes.newDates && (
                      <div className="mt-4 bg-[#FF6B35]/10 border border-[#FF6B35] rounded-lg p-4">
                        ...
                        <div className="mt-2 text-xs text-gray-600">
                          💡 <strong>Tip:</strong> Click on the date field and select both date AND time. Make sure to set a specific time, not just the date.
                        </div>
                        ...
                        {postponementOptions?.data && (
                          <div className="mt-2 text-xs text-blue-600 bg-blue-50 p-2 rounded">
                            ...
                            <ul className="mt-1 space-y-1">
                              <li>• Minimum notice: {postponementOptions.data.noticeRequired} days from today</li>
                              <li>• Maximum postponement: {postponementOptions.data.maxPostponementDuration} days from today</li>
                              <li>• End date must be same day or after start date</li>
                            </ul>
                          </div>
                        )}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          ...
                          <div className="relative">
                            <select
                              ...
                              onChange={(e) => handleNewMeetingDataChange('meetingPlatform', e.target.value)}
                              className="w-full px-3 py-3 border border-[#FF6B35] rounded-md focus:ring-2 focus:ring-[#FF6B35] focus:border-[#FF6B35] outline-none transition-colors text-sm bg-white"
                            >
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-4">
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) => handleNewMeetingDataChange('meetingLink', e.target.value)}
                              className="w-full px-3 py-3 border border-[#FF6B35] rounded-md focus:ring-2 focus:ring-[#FF6B35] focus:border-[#FF6B35] outline-none transition-colors text-sm bg-white"
                            />
                            <label className="absolute -top-2 left-3 px-2 text-xs font-medium text-[#FF6B35] bg-white">
                              Meeting Link
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) => handleNewMeetingDataChange('meetingId', e.target.value)}
                              className="w-full px-3 py-3 border border-[#FF6B35] rounded-md focus:ring-2 focus:ring-[#FF6B35] focus:border-[#FF6B35] outline-none transition-colors text-sm bg-white"
                            />
                            <label className="absolute -top-2 left-3 px-2 text-xs font-medium text-[#FF6B35] bg-white">
                              Meeting ID
                        ...
                        <div className="grid grid-cols-2 gap-4 mt-4">
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) => handleNewMeetingDataChange('passcode', e.target.value)}
                              className="w-full px-3 py-3 border border-[#FF6B35] rounded-md focus:ring-2 focus:ring-[#FF6B35] focus:border-[#FF6B35] outline-none transition-colors text-sm bg-white"
                            />
                            <label className="absolute -top-2 left-3 px-2 text-xs font-medium text-[#FF6B35] bg-white">
                              Passcode
                          ...
                          <div className="relative">
                            <input
                              ...
                              onChange={(e) => handleNewMeetingDataChange('participantInstructions', e.target.value)}
                              className="w-full px-3 py-3 border border-[#FF6B35] rounded-md focus:ring-2 focus:ring-[#FF6B35] focus:border-[#FF6B35] outline-none transition-colors text-sm bg-white"
                            />
                            <label className="absolute -top-2 left-3 px-2 text-xs font-medium text-[#FF6B35] bg-white">
                              Instructions
                      ...
                      <div className="mt-4 bg-[#FF6B35]/10 border border-[#FF6B35] rounded-lg p-4">
                        ...
                        <div className="space-y-2">
                          <input
                            type="range"
                            min="0"
                            max={postponementOptions?.data?.maxRefundPercentage || 100}
                          ...
                          <div className="flex justify-between text-sm text-gray-600">
                            <span>0%</span>
                            <span className="font-medium text-[#FF6B35]">{refundPercentage}%</span>
                            <span>{postponementOptions?.data?.maxRefundPercentage || 100}%</span>
                          </div>
                        </div>
                      </div>
                    )}
              ...
              <div className="flex justify-end space-x-3 mt-6">
                ...
                <button
                  ...
                  disabled={isLoading || isPostponing}
                ...
                >
                  ...
                  {isLoading || isPostponing ? 'Postponing...' : 'Postpone Event'}
                </button>
              </div>
            </div>
          </div>
        ...
        details={[
          {
            ...
            value: postponementResult?.newDates?.startDate 
              ? new Date(postponementResult.newDates.startDate).toLocaleDateString('en-US', {
                  weekday: 'long',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
          ...
          },
          {
            ...
            value: postponementResult?.newDates?.endDate 
              ? new Date(postponementResult.newDates.endDate).toLocaleDateString('en-US', {
                  weekday: 'long',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
