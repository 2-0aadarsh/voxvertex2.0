...
{{organizer_name}}`
  ...
  const handleTemplateChange = (templateName: string) => {
    const template = messageTemplates[templateName as keyof typeof messageTemplates];
    setMessageForm(prev => ({
      ...prev,
        ...
        <div className="bg-white rounded-2xl border border-[#FF6B35] p-6">
          ...
          {scheduledMessages.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-xs">Send your first message to participants above</p>
            </div>
          ) : (
            <div className="py-6 space-y-4">
              {scheduledMessages.map((message) => (
                <div key={message.id} className="border border-[#FF6B35] rounded-lg p-4 text-left">
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-gray-900 font-medium text-sm">{message.subject || 'Untitled Message'}</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-3xl shadow-lg max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-8">
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h2 className="text-2xl font-bold text-[#FF6B35]">Send Message to Participants</h2>
                  <p className="text-gray-600 mt-2">Create and send messages to your event participants, speakers, and team members.</p>
                </div>
                <button
                        ...
                        {Object.keys(messageTemplates).map((templateName) => (
                          <div
                            ...
                            className={`px-4 py-3 text-xs cursor-pointer hover:bg-[#FF6B35]/10 transition-colors ${
                              messageForm.template === templateName ? 'bg-[#FF6B35]/5 text-[#FF6B35]' : 'text-gray-700'
                            }`}
                          >
                            {templateName}
                          </div>
                    ...
                    {showCalendar && (
                      <div className="absolute top-full left-0 mt-1 bg-white border border-gray-300 rounded-lg shadow-lg z-50 p-4">
                        <div className="text-center mb-3">
                          <h4 className="text-sm font-medium text-[#FF6B35] mb-2">Select Date & Time</h4>
                        </div>
                        ...
                        <div className="mb-3">
                          ...
                          <input
                            type="date"
                            value={messageForm.scheduleDate.split('-').reverse().join('-')}
                            onChange={(e) => {
                        ...
                        <div className="mb-3">
                          ...
                          <input
                            type="time"
                            value={messageForm.scheduleTime.replace(' AM', '').replace(' PM', '')}
                            onChange={(e) => {
              ...
              {activeTab === 'recipients' && (
                <div className="space-y-6">
                  <h3 className="text-[#FF6B35] text-sm font-medium mb-6">Select Recipients</h3>
                  
                  <div className="space-y-4">
                    {/* All Participants Option */}
                    <div className={`flex items-center p-4 border-2 rounded-lg transition-colors ${
                      ...
                      <input 
                        ...
                        checked={selectedRecipients.includes('all-participants')}
                    ...
                    {selectedRecipients.includes('all-participants') && (
                      <div className="ml-7 space-y-3">
                        <h4 className="text-[#FF6B35] text-sm font-medium">Filter by ticket type:</h4>
                        ...
                        <div className="flex items-center p-3 border border-gray-300 rounded-lg hover:border-[#FF6B35] transition-colors">
                          <input 
                            ...
                            onChange={() => handleTicketTypeChange('early-bird')}
                            className="w-4 h-4 text-[#FF6B35] focus:ring-[#FF6B35] focus:ring-2 rounded"
                          />
                          <label htmlFor="early-bird" className="ml-3 text-xs text-gray-900">
                            Early Bird (1)
                          </label>
                        </div>

                        {/* Regular Ticket */}
                        <div className={`flex items-center p-3 border-2 rounded-lg transition-colors ${
                          ...
                          <input 
                            ...
                            className="w-4 h-4 text-[#FF6B35] focus:ring-[#FF6B35] focus:ring-2 rounded"
                          />
                          <label htmlFor="regular" className="ml-3 text-xs text-gray-900">
                            Regular (2)
                          </label>
                        </div>
                      </div>
                    ...
                    <div className={`flex items-center p-4 border rounded-lg transition-colors ${
                      ...
                      <input 
                        ...
                        className="w-4 h-4 text-[#FF6B35] focus:ring-[#FF6B35] focus:ring-2 rounded"
                      />
                      <label htmlFor="speakers" className="ml-3 text-xs text-gray-900">
                        Speaker (2)
                      </label>
                    </div>
                  </div>
                  ...
                  <div className="mt-8 p-4 bg-[#FF6B35]/10 border border-[#FF6B35] rounded-lg flex justify-between items-center">
                    <span className="text-[#FF6B35] text-sm font-medium">Total Recipients:</span>
              ...
              {activeTab === 'preview' && (
                <div className="space-y-6">
                  ...
                  <p className="text-xs text-gray-600">
                    * This preview shows how the message will appear with variables replaced. Individual participant details will be personalized for each recipient.
                  </p>
                </div>
              )}
