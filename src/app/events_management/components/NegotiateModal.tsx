    ...
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-[#FF6B35]">Negotiate New Date</h2>
          <button 
            onClick={onClose}
        ...
        <div className="p-6">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-yellow-900 mb-1">Event Postponed</h3>
                <p className="text-sm text-yellow-800 mb-1">{event.title}</p>
                <div className="flex items-center gap-2 text-sm text-yellow-700 mb-1">
                  <Calendar className="w-4 h-4" />
                  <span>Original Date: {event.date}</span>
                </div>
                <p className="text-sm text-yellow-700">
                  <span className="font-medium">Reason:</span> Speaker availability conflict - requesting new date options
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <label className="flex items-start gap-3 p-4 border border-gray-200 rounded-lg mb-3 cursor-pointer hover:bg-gray-50 transition-colors">
              ...
              />
              <div>
                <p className="font-medium text-green-700">Accept Postponement</p>
                <p className="text-sm text-gray-600">Accept postponement and propose a new date</p>
              </div>
            </label>
            ...
            <label className="flex items-start gap-3 p-4 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Unable to accommodate the new schedule</p>
              </div>
            </label>
          </div>
          ...
          {postponeResponse === 'accept' && (
            <div className="mb-6">
              <p className="text-sm font-medium text-gray-900 mb-3">Propose New Date & Time</p>
          ...
          {postponeResponse === 'decline' && (
            <div className="mb-6">
              <p className="text-sm font-medium text-gray-900 mb-2">Reason for Declining (Optional)</p>
              <textarea 
                value={declineReason}
          ...
          <div className="bg-orange-50 border border-[#FF6B35] rounded-lg p-4 mb-6">
            <h3 className="font-semibold text-[#FF6B35] mb-2">Important Notes</h3>
            <ul className="text-sm text-[#FF6B35] space-y-1">
              <li>• Your response will be sent to {event.organizer} at Global Connect Events</li>
              <li>• If declining, the event may be cancelled and you can propose a settlement fee</li>
              <li>• If accepting, ensure the new date works with your schedule</li>
              <li>• You can provide a rating and feedback regardless of your decision</li>
            </ul>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button 
              onClick={onSubmit}
              disabled={!postponeResponse || (postponeResponse === 'accept' && (!proposedDate || !proposedTime))}
            ...
            >
              {postponeResponse === 'accept' ? 'Accept Postponement' : postponeResponse === 'decline' ? 'Decline Postponement' : 'Select Response'}
            </button>
          </div>
        </div>
      </div>
