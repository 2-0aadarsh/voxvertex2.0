    ...
    <div className="fixed inset-0 bg-[#FF6B35]/20 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 p-4 flex items-center justify-between">
          <h2 className="text-xl font-semibold text-[#FF6B35]">Propose Settlement Fee</h2>
          <button 
            onClick={onClose}
        ...
        <div className="p-6">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
            <div className="flex items-start gap-2">
              ...
              <div>
                <h3 className="font-semibold text-red-800 mb-1">Event Cancelled</h3>
                <p className="text-sm text-red-700 mb-1">{event.title} organized by TechCorp Events</p>
                <p className="text-sm text-red-700">Original fee: ${event.amount.toLocaleString()}</p>
              </div>
            </div>
          </div>

          <div className="mb-4">
            <label className="block text-sm font-medium text-[#FF6B35] mb-2">Settlement Amount</label>
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2">
              ...
              >
                10% (${(event.amount * 0.1).toLocaleString()})
              </button>
              <button 
                onClick={() => onQuickSelect(25)}
                className="px-4 py-2 bg-gray-100 hover:bg-[#FF6B35]/10 hover:text-[#FF6B35] text-gray-700 rounded-lg text-sm font-medium transition-colors"
              >
                25% (${(event.amount * 0.25).toLocaleString()})
              </button>
              <button 
                onClick={() => onQuickSelect(50)}
                className="px-4 py-2 bg-gray-100 hover:bg-[#FF6B35]/10 hover:text-[#FF6B35] text-gray-700 rounded-lg text-sm font-medium transition-colors"
              >
                50% (${(event.amount * 0.5).toLocaleString()})
              </button>
            </div>
          </div>

          <div className="mb-6">
            <label className="block text-sm font-medium text-[#FF6B35] mb-2">Reason for Settlement</label>
          ...
          <div className="bg-orange-50 border border-[#FF6B35] rounded-lg p-4 mb-6">
            <h3 className="font-semibold text-[#FF6B35] mb-2">Settlement Guidelines</h3>
            <ul className="text-sm text-[#FF6B35] space-y-1">
              <li>• Settlement fees typically range from 10-50% of the original fee</li>
              <li>• Consider preparation time, materials developed, and opportunity cost</li>
